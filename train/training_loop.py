    ...
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        if args.stage == "full-text":
            for name, param in self.model.named_parameters():
                if 'prior_network' in name or 'mlp_fc' in name:  # 根据需要选择
                    params_to_optimize.append(param)
            self.opt = AdamW(params_to_optimize, lr=self.lr, weight_decay=self.weight_decay)
        elif args.stage == "only-actions":
            for name, param in self.model.named_parameters():
                if 'prior_network' in name or 'mlp_fc.atomic_action_branch' in name:  # 根据需要选择
                    params_to_optimize.append(param)
            self.opt = AdamW(params_to_optimize, lr=self.lr, weight_decay=self.weight_decay)
        elif args.stage == "wo-physics":
            for name, param in self.model.named_parameters():
                if 'prior_network' in name or 'mlp_fc.latent_feature_branch' in name:  # 根据需要选择
                    params_to_optimize.append(param)
            self.opt = AdamW(params_to_optimize, lr=self.lr, weight_decay=self.weight_decay)
        else:
            self.opt = AdamW(
