...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list):
            ...
            # # 使用整行作为 caption
            # caption = line.strip()  # 不再分割，直接使用整行内容作为 caption
            # if "/" in caption:
            #     caption = caption.replace("/", " ")

            # # 为每个单词后面加上 "/VERB"
            # tokens = [word + "/VERB" for word in caption.split()]
            # f_tag = 0.0
            # to_tag = 0.0

            # text_dict['caption'] = caption
            # text_dict['tokens'] = tokens  # 设置为 null 或者固定格式
    ...
    def __getitem__(self, item):
        ...
        # Randomly select a caption
        text_data = random.choice(text_list)
        # print(f"Selected text data: {text_data}")
        caption, tokens = text_data['caption'], text_data['tokens']
        ...
        for token in tokens:
            ...
            if not token:  # 如果 token 为空，则跳过该 token
                # print(f"text data in the empty token: {text_data}")
                continue
            try:
                word_emb, pos_oh = self.w_vectorizer[token]
            except Exception as e:
                print(f"Error processing token: {token}")
                print(f"Selected text data: {text_data}")
                continue  # 跳过该 token，继续处理下一个
            pos_one_hots.append(pos_oh[None, :])
            word_embeddings.append(word_emb[None, :])
        pos_one_hots = np.concatenate(pos_one_hots, axis=0)
        word_embeddings = np.concatenate(word_embeddings, axis=0)
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='/home/jovyan/mogen/motion-diffusion-model/dataset/humanml_opt.txt', split="train", **kwargs):
        self.mode = mode
        ...
        abs_base_path = f'/home/jovyan/mogen/motion-diffusion-model'
